...
public:
    ...
    void interaction(Particle& p1 ,Particle& p2){
        ...
        double force = (p1.charge + p1.alcoholism) * (p2.charge + p2.alcoholism);
        force = force / r / r;
...
public:
    ...
    int chetchik = 0;
    // в этом конструкторе предлагается задавать координаты и через энергию остальные параметры частицы
    explicit Proton(vector3 velocity, vector3 coords){
        this->velocity = velocity;
        this->coordinate = coords;
        this->mass = 2000;
        this->charge = 1;
        ...
        Number++;
        chetchik++;
        this->name = "Proton #" + std::to_string(chetchik);
    };
    ~Proton(){
        Number--;
    };
...
class Dimon : public Particle {
public:
    int Number = 0;
    int chetchik = 0;
    // в этом конструкторе предлагается задавать координаты и через энергию остальные параметры частицы
    explicit Dimon(vector3 velocity, vector3 coords){
        this->velocity = velocity;
        this->coordinate = coords;
        this->mass = 20000000;
        this->charge = 0;
        this->alcoholism = (rand() % 10);
        Number++;
        chetchik++;
        this->name = "Dimon #" + std::to_string(chetchik);
    };
    ~Dimon(){
        Number--;
    };
...
public:
    ...
    void addParticle(int type,vector3 cord,vector3 speed){
        // я тут какую-то чушь написала, так что у меня при любом раскладе выводился
        // нейтрон, протон и димон. Потом постараюсь написать заново
    };
